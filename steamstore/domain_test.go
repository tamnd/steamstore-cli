...
// HTTP behaviour is covered in steamstore_test.go.

func TestDomainInfo(t *testing.T) {
	info := Domain{}.Info()
	if info.Scheme != "steamstore" {
		t.Errorf("Scheme = %q, want steamstore", info.Scheme)
	}
...
func TestHostWiring(t *testing.T) {
	...
	if want := "steamstore://page/wiki/Go"; u.String() != want {
	...
	got, err := h.ResolveOn("steamstore", "about")
	if err != nil || got.String() != "steamstore://page/about" {
		t.Errorf("ResolveOn = (%q, %v), want steamstore://page/about", got.String(), err)
	}
}
