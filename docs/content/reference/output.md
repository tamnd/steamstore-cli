...
describes what users get. Pick a format with `-o`, or let steam choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
steam <command> -o table   # aligned columns for reading
steam <command> -o jsonl   # one JSON object per line, for piping
steam <command> -o json    # a single JSON array
steam <command> -o csv     # spreadsheet friendly
steam <command> -o tsv     # tab-separated
steam <command> -o url     # just the URL column
steam <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
steam <command> --fields id,title,url
```
...
```bash
steam <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
steam <command>            # a table, because this is a terminal
steam <command> | wc -l    # JSONL, because this is a pipe
```
